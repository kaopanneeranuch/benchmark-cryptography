...
/* rcon_row1 is XOR'd into word 0 of every round sub-tweakey (constant). */
#define RCON_ROW1  0x01020408U

/* GETRCON(r): rcon[r] replicated across all 4 bytes, XOR'd into word 1. */
#define GETRCON(r) (((uint32_t)rcon_tab[r]<<24)^((uint32_t)rcon_tab[r]<<16) \
                   ^((uint32_t)rcon_tab[r]<< 8)^ (uint32_t)rcon_tab[r])

/* ------------------------------------------------------------------ */
/* Deoxys round constants (rounds 0..14, BC-256 only needs 0..14)      */
/* ------------------------------------------------------------------ */

static const uint8_t rcon_tab[15] = {
    0x2f, 0x5e, 0xbc, 0x63, 0xc6, 0x97, 0x35, 0x6a,
    0xd4, 0xb3, 0x7d, 0xfa, 0xef, 0xc5, 0x91
};

/* ------------------------------------------------------------------ */
/* H permutation (byte positions)                                      */
/* perm[i] = source byte index for output byte i                       */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* LFSR-2 table for TK1 update (G function with alpha=2)               */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* AES T-tables (encryption, standard Rijndael T-table form)           */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Tweakey step helpers                                                 */
/* ------------------------------------------------------------------ */

/* TK1: apply H (byte permutation) then G2 (lfsr2 each byte) */
static inline void tk1_step(uint8_t t[16])
{
    uint8_t n0  = lfsr2[t[perm[ 0]]],  n1  = lfsr2[t[perm[ 1]]],
            n2  = lfsr2[t[perm[ 2]]],  n3  = lfsr2[t[perm[ 3]]],
            n4  = lfsr2[t[perm[ 4]]],  n5  = lfsr2[t[perm[ 5]]],
            n6  = lfsr2[t[perm[ 6]]],  n7  = lfsr2[t[perm[ 7]]],
            n8  = lfsr2[t[perm[ 8]]],  n9  = lfsr2[t[perm[ 9]]],
            n10 = lfsr2[t[perm[10]]], n11 = lfsr2[t[perm[11]]],
            n12 = lfsr2[t[perm[12]]], n13 = lfsr2[t[perm[13]]],
            n14 = lfsr2[t[perm[14]]], n15 = lfsr2[t[perm[15]]];
    t[ 0]=n0;  t[ 1]=n1;  t[ 2]=n2;  t[ 3]=n3;
    t[ 4]=n4;  t[ 5]=n5;  t[ 6]=n6;  t[ 7]=n7;
    t[ 8]=n8;  t[ 9]=n9;  t[10]=n10; t[11]=n11;
    t[12]=n12; t[13]=n13; t[14]=n14; t[15]=n15;
}

/* TK2: apply H only (alpha=1 => identity LFSR, just permute) */
static inline void tk2_step(uint8_t t[16])
{
    uint8_t n0  = t[perm[ 0]], n1  = t[perm[ 1]],
            n2  = t[perm[ 2]], n3  = t[perm[ 3]],
            n4  = t[perm[ 4]], n5  = t[perm[ 5]],
            n6  = t[perm[ 6]], n7  = t[perm[ 7]],
            n8  = t[perm[ 8]], n9  = t[perm[ 9]],
            n10 = t[perm[10]], n11 = t[perm[11]],
            n12 = t[perm[12]], n13 = t[perm[13]],
            n14 = t[perm[14]], n15 = t[perm[15]];
    t[ 0]=n0;  t[ 1]=n1;  t[ 2]=n2;  t[ 3]=n3;
    t[ 4]=n4;  t[ 5]=n5;  t[ 6]=n6;  t[ 7]=n7;
    t[ 8]=n8;  t[ 9]=n9;  t[10]=n10; t[11]=n11;
    t[12]=n12; t[13]=n13; t[14]=n14; t[15]=n15;
...
/* ------------------------------------------------------------------ */
/* AES round macro (ShiftRows + MixColumns + SubBytes via T-tables)    */
/* Input: s0-s3; output: t0-t3; adds rk words w0-w3.                  */
/* ------------------------------------------------------------------ */

#define AES_ROUND(s0,s1,s2,s3, t0,t1,t2,t3, w0,w1,w2,w3) do { \
    (t0) = Te0[(s0)>>24] ^ Te1[((s1)>>16)&0xff]               \
         ^ Te2[((s2)>> 8)&0xff] ^ Te3[(s3)&0xff] ^ (w0);      \
    (t1) = Te0[(s1)>>24] ^ Te1[((s2)>>16)&0xff]               \
         ^ Te2[((s3)>> 8)&0xff] ^ Te3[(s0)&0xff] ^ (w1);      \
    (t2) = Te0[(s2)>>24] ^ Te1[((s3)>>16)&0xff]               \
         ^ Te2[((s0)>> 8)&0xff] ^ Te3[(s1)&0xff] ^ (w2);      \
    (t3) = Te0[(s3)>>24] ^ Te1[((s0)>>16)&0xff]               \
         ^ Te2[((s1)>> 8)&0xff] ^ Te3[(s2)&0xff] ^ (w3);      \
} while(0)

/* ------------------------------------------------------------------ */
/* Build one STK block and return its 4 words                          */
/* ------------------------------------------------------------------ */

#define STK_WORDS(t1,t2,r, w0,w1,w2,w3) do {              \
    (w0) = GETU32((t1))    ^ GETU32((t2))    ^ RCON_ROW1; \
    (w1) = GETU32((t1)+4)  ^ GETU32((t2)+4)  ^ GETRCON(r);\
    (w2) = GETU32((t1)+8)  ^ GETU32((t2)+8);              \
...
void deoxysbc256_precompute_tk1(deoxysbc256_ctx_t *ctx, const uint8_t tk1[16])
{
    ...
    memcpy(t, tk1, 16);
    for (int r = 0; r < 15; r++) {
        ctx->rtk1[4*r+0] = GETU32(t)    ^ RCON_ROW1;
        ctx->rtk1[4*r+1] = GETU32(t+4)  ^ GETRCON(r);
        ctx->rtk1[4*r+2] = GETU32(t+8);
        ctx->rtk1[4*r+3] = GETU32(t+12);
        tk1_step(t);
    }
}
                         ...
                         uint8_t ct[16])
{
    uint8_t t2[16];
    uint32_t rk[60];
    uint32_t s0, s1, s2, s3, t0, t1, t2w, t3;

    memcpy(t2, tk2, 16);
    for (int r = 0; r < 15; r++) {
        rk[4*r+0] = ctx->rtk1[4*r+0] ^ GETU32(t2);
        rk[4*r+1] = ctx->rtk1[4*r+1] ^ GETU32(t2+4);
        rk[4*r+2] = ctx->rtk1[4*r+2] ^ GETU32(t2+8);
        rk[4*r+3] = ctx->rtk1[4*r+3] ^ GETU32(t2+12);
        tk2_step(t2);
    }
    ...
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2w,t3, rk[ 4],rk[ 5],rk[ 6],rk[ 7]);
    AES_ROUND(t0,t1,t2w,t3, s0,s1,s2,s3, rk[ 8],rk[ 9],rk[10],rk[11]);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2w,t3, rk[12],rk[13],rk[14],rk[15]);
    AES_ROUND(t0,t1,t2w,t3, s0,s1,s2,s3, rk[16],rk[17],rk[18],rk[19]);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2w,t3, rk[20],rk[21],rk[22],rk[23]);
    AES_ROUND(t0,t1,t2w,t3, s0,s1,s2,s3, rk[24],rk[25],rk[26],rk[27]);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2w,t3, rk[28],rk[29],rk[30],rk[31]);
    AES_ROUND(t0,t1,t2w,t3, s0,s1,s2,s3, rk[32],rk[33],rk[34],rk[35]);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2w,t3, rk[36],rk[37],rk[38],rk[39]);
    AES_ROUND(t0,t1,t2w,t3, s0,s1,s2,s3, rk[40],rk[41],rk[42],rk[43]);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2w,t3, rk[44],rk[45],rk[46],rk[47]);
    AES_ROUND(t0,t1,t2w,t3, s0,s1,s2,s3, rk[48],rk[49],rk[50],rk[51]);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2w,t3, rk[52],rk[53],rk[54],rk[55]);
    AES_ROUND(t0,t1,t2w,t3, s0,s1,s2,s3, rk[56],rk[57],rk[58],rk[59]);

    PUTU32(ct,    s0); PUTU32(ct+4,  s1);
                              ...
                              uint8_t ct[16])
{
    ...
    STK_WORDS(tk1, tk2, 0, w0, w1, w2, w3);
    tk1_step(tk1); tk2_step(tk2);
    s0 = GETU32(pt)    ^ w0;
    s1 = GETU32(pt+4)  ^ w1;
    s2 = GETU32(pt+8)  ^ w2;
    s3 = GETU32(pt+12) ^ w3;
    ...
    STK_WORDS(tk1,tk2,2,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(t0,t1,t2,t3, s0,s1,s2,s3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,3,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2,t3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,4,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(t0,t1,t2,t3, s0,s1,s2,s3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,5,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2,t3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,6,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(t0,t1,t2,t3, s0,s1,s2,s3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,7,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2,t3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,8,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(t0,t1,t2,t3, s0,s1,s2,s3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,9,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2,t3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,10,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(t0,t1,t2,t3, s0,s1,s2,s3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,11,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2,t3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,12,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(t0,t1,t2,t3, s0,s1,s2,s3, w0,w1,w2,w3);

    STK_WORDS(tk1,tk2,13,w0,w1,w2,w3); tk1_step(tk1); tk2_step(tk2);
    AES_ROUND(s0,s1,s2,s3, t0,t1,t2,t3, w0,w1,w2,w3);

    /* Round 14: final round, result lands in s */
    STK_WORDS(tk1,tk2,14,w0,w1,w2,w3);
    AES_ROUND(t0,t1,t2,t3, s0,s1,s2,s3, w0,w1,w2,w3);
