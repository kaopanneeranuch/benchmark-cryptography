...
static const uint8_t rcon[17] = {0x2f,0x5e,0xbc,0x63,0xc6,0x97,0x35,0x6a, 0xd4,0xb3,0x7d,0xfa,0xef,0xc5,0x91,0x39, 0x72};
...
uint8_t choose_lfsr (uint8_t x, uint8_t alpha) {
  if( 1 == alpha ) return x;
  if( 2 == alpha ) return lfsr2[x];//mul2[x];
  if( 3 == alpha ) return lfsr4[x];//mul2[mul2[x]];
...
void H (uint8_t tweakey[]) {
  int i;
  uint8_t tmp[16];
  for(i = 0; i<16; i++) tmp[i] = tweakey[perm[i]];
...
*/
int deoxysKeySetupEnc(uint32_t* rtweakey,
...
*/
int deoxysKeySetupDec(uint32_t* rtweakey, 
                         const uint8_t* TweakKey,
                         int no_tweakeys)
{
    ...
    Nr = deoxysKeySetupEnc(rtweakey, TweakKey, no_tweakeys);
    ...
    for (i = 1; i <= Nr; i++) {
        rtweakey += 4;
        rtweakey[0] = Td0[Te4[(rtweakey[0] >> 24)] & 0xff] ^ Td1[Te4[(rtweakey[0] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rtweakey[0] >>  8) & 0xff] & 0xff] ^ Td3[Te4[(rtweakey[0]) & 0xff] & 0xff];
        rtweakey[1] = Td0[Te4[(rtweakey[1] >> 24)] & 0xff] ^ Td1[Te4[(rtweakey[1] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rtweakey[1] >>  8) & 0xff] & 0xff] ^ Td3[Te4[(rtweakey[1]) & 0xff] & 0xff];
        rtweakey[2] = Td0[Te4[(rtweakey[2] >> 24)] & 0xff] ^ Td1[Te4[(rtweakey[2] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rtweakey[2] >>  8) & 0xff] & 0xff] ^ Td3[Te4[(rtweakey[2]) & 0xff] & 0xff];
        rtweakey[3] = Td0[Te4[(rtweakey[3] >> 24)] & 0xff] ^ Td1[Te4[(rtweakey[3] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rtweakey[3] >>  8) & 0xff] & 0xff] ^ Td3[Te4[(rtweakey[3]) & 0xff] & 0xff];
    }
    return Nr;
}
...
void aesTweakEncrypt(uint32_t tweakey_size,
    ...
    deoxysKeySetupEnc(rk, key, tweakey_size/128);
                     ...
                     uint8_t pt[16])
{
    ...
    /* Produce the round tweakeys */
    deoxysKeySetupDec(rk, key, tweakey_size/128);
