 ...
 * Uses the LEFT fork output as the block-cipher ciphertext.
 */
                              ...
                              const uint8_t in[16], uint8_t out[16])
{
    ...
    uint8_t discard_c1[16];
    forkskinny_128_256_encrypt(tk, out, discard_c1, in);
}
                            ...
                            const uint8_t *pt, size_t len, uint8_t *ct)
{
    ...
    while (off < len) {
        inc_be32(ctr);
        /* Compute both branches so the cost matches full fork usage. */
        forkskinny_128_256_encrypt(tk, keystream, c1, ctr);
...
int forkskinny_gcm_decrypt(const uint8_t key[16], const uint8_t nonce[12],
                           const uint8_t *aad, size_t aad_len,
