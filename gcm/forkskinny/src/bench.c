...
{
    printk("--- FORKSKINNY GCM Correctness (msg=%d ad=%d) ---\n",
           MESSAGE_LEN, AD_LEN);
    ...
    forkskinny_ctr_encrypt(bench_key, bench_nonce, pt_buf, MESSAGE_LEN, ct_buf);

    /* produce ciphertext + tag using GCM API */
    forkskinny_gcm_encrypt(bench_key, bench_nonce, 12,
                           ad_buf, AD_LEN,
                           pt_buf, MESSAGE_LEN,
                           ct_buf, tag_buf);
    ...
    int rc = forkskinny_gcm_decrypt(bench_key, bench_nonce, 12,
                                    ad_buf, AD_LEN,
                                    ct_buf, MESSAGE_LEN,
                                    tag_buf, dec_buf);
...
static void bench_keygen(void)
{
    ...
    for (int i = 0; i < WARMUP_ITERS; i++)
        forkskinny_gcm_keygen(bench_key);
...
static void bench_hash(void)
{
    ...
    uint8_t H[16];
    uint8_t EkJ0[16];
    forkskinny_gcm_compute_H(bench_key, H);
    forkskinny_gcm_compute_EkJ0(bench_key, bench_nonce, EkJ0);
...
static void bench_decrypt(void)
{
    ...
    for (int i = 0; i < WARMUP_ITERS; i++)
        forkskinny_ctr_encrypt(bench_key, bench_nonce, ct_buf, MESSAGE_LEN, dec_buf);
