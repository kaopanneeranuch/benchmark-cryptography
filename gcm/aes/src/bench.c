...
static uint32_t probe_128_block_calls_auth_only(size_t msg_len)
{
    ...
    aes_128_gcm_auth(bench_key_128, bench_nonce, AES_GCM_NONCE_LEN,
                     ad_buf, AD_LEN, ct_buf, msg_len, tag_buf);
    return aes_128_gcm_get_block_calls();
}

static uint32_t probe_128_block_calls_verify_only(size_t msg_len)
{
    aes_gcm_counters_reset();
...
static uint32_t probe_128_block_calls_encrypt_only(size_t msg_len)
{
    aes_gcm_counters_reset();
    aes_128_gcm_ctr_crypt(bench_key_128, bench_nonce, pt_buf, msg_len, ct_buf);
    return aes_128_gcm_get_block_calls();
}

static uint32_t probe_128_block_calls_decrypt_only(size_t msg_len)
{
    aes_gcm_counters_reset();
    aes_128_gcm_ctr_crypt(bench_key_128, bench_nonce, ct_buf, msg_len, dec_buf);
    return aes_128_gcm_get_block_calls();
}

static uint32_t probe_256_block_calls_auth_only(size_t msg_len)
{
    aes_gcm_counters_reset();
...
static uint32_t probe_256_block_calls_verify_only(size_t msg_len)
{
    aes_gcm_counters_reset();
...
static uint32_t probe_256_block_calls_encrypt_only(size_t msg_len)
{
    aes_gcm_counters_reset();
    aes_256_gcm_ctr_crypt(bench_key_256, bench_nonce, pt_buf, msg_len, ct_buf);
    return aes_256_gcm_get_block_calls();
}

static uint32_t probe_256_block_calls_decrypt_only(size_t msg_len)
{
    aes_gcm_counters_reset();
    aes_256_gcm_ctr_crypt(bench_key_256, bench_nonce, ct_buf, msg_len, dec_buf);
    return aes_256_gcm_get_block_calls();
}

static uint32_t probe_rijndael256_block_calls_auth_only(size_t msg_len)
{
    ...
    rijndael256_gcm_auth(bench_key_rijndael256, bench_nonce_rijndael256, RIJNDAEL256_GCM_NONCE_LEN,
                         ad_buf, AD_LEN, ct_buf, msg_len, tag_buf_r256);
    return rijndael256_gcm_get_block_calls();
}

static uint32_t probe_rijndael256_block_calls_verify_only(size_t msg_len)
{
    rijndael256_gcm_counters_reset();
...
static uint32_t probe_rijndael256_block_calls_encrypt_only(size_t msg_len)
{
    rijndael256_gcm_counters_reset();
    rijndael256_gcm_ctr_crypt(bench_key_rijndael256, bench_nonce_rijndael256,
                               pt_buf, msg_len, ct_buf);
    return rijndael256_gcm_get_block_calls();
}

static uint32_t probe_rijndael256_block_calls_decrypt_only(size_t msg_len)
{
    rijndael256_gcm_counters_reset();
    rijndael256_gcm_ctr_crypt(bench_key_rijndael256, bench_nonce_rijndael256,
                               ct_buf, msg_len, dec_buf);
    return rijndael256_gcm_get_block_calls();
}
...
static void bench_128_auth_only(size_t msg_len)
{
    ...
    aes_128_gcm_ctr_crypt(bench_key_128, bench_nonce, pt_buf, msg_len, ct_buf);
    block_calls = probe_128_block_calls_auth_only(msg_len);
...
static void bench_128_verify_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    fill_pattern(ad_buf, AD_LEN);
    aes_128_gcm_encrypt_auth(bench_key_128, bench_nonce, AES_GCM_NONCE_LEN,
                             ad_buf, AD_LEN, pt_buf, msg_len, ct_buf, tag_buf);
    block_calls = probe_128_block_calls_verify_only(msg_len);
...
static void bench_128_encrypt_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    fill_pattern(pt_buf, msg_len);
    block_calls = probe_128_block_calls_encrypt_only(msg_len);
...
static void bench_128_decrypt_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    fill_pattern(pt_buf, msg_len);
    aes_128_gcm_ctr_crypt(bench_key_128, bench_nonce, pt_buf, msg_len, ct_buf);
    block_calls = probe_128_block_calls_decrypt_only(msg_len);
...
static void bench_128_psa_encrypt(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    psa_key_id_t key_id;
    static uint8_t out[MAX_MSG_LEN + AES_GCM_TAG_LEN];
    size_t out_len = 0;
...
static void bench_128_psa_decrypt(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    psa_key_id_t key_id;
    static uint8_t enc[MAX_MSG_LEN + AES_GCM_TAG_LEN];
    static uint8_t dec[MAX_MSG_LEN];
...
static void bench_256_auth_only(size_t msg_len)
{
    ...
    fill_pattern(ad_buf, AD_LEN);
    aes_256_gcm_ctr_crypt(bench_key_256, bench_nonce, pt_buf, msg_len, ct_buf);
    block_calls = probe_256_block_calls_auth_only(msg_len);
...
static void bench_256_verify_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    aes_256_gcm_encrypt_auth(bench_key_256, bench_nonce, AES_GCM_NONCE_LEN,
                             ad_buf, AD_LEN, pt_buf, msg_len, ct_buf, tag_buf);
    block_calls = probe_256_block_calls_verify_only(msg_len);
...
static void bench_256_encrypt_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    fill_pattern(pt_buf, msg_len);
    block_calls = probe_256_block_calls_encrypt_only(msg_len);
...
static void bench_256_decrypt_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    fill_pattern(pt_buf, msg_len);
    aes_256_gcm_ctr_crypt(bench_key_256, bench_nonce, pt_buf, msg_len, ct_buf);
    block_calls = probe_256_block_calls_decrypt_only(msg_len);
...
static void bench_256_psa_encrypt(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    psa_key_id_t key_id;
...
static void bench_256_psa_decrypt(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    psa_key_id_t key_id;
...
static void bench_rijndael256_auth_only(size_t msg_len)
{
    ...
    rijndael256_gcm_ctr_crypt(bench_key_rijndael256, bench_nonce_rijndael256,
                               pt_buf, msg_len, ct_buf);
    block_calls = probe_rijndael256_block_calls_auth_only(msg_len);
...
static void bench_rijndael256_verify_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    block_calls = probe_rijndael256_block_calls_verify_only(msg_len);
...
static void bench_rijndael256_encrypt_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    fill_pattern(pt_buf, msg_len);
    block_calls = probe_rijndael256_block_calls_encrypt_only(msg_len);
...
static void bench_rijndael256_decrypt_only(size_t msg_len)
{
    timing_t start, end;
    uint64_t total_c = 0, total_ns = 0;
    uint32_t block_calls;
    ...
    rijndael256_gcm_ctr_crypt(bench_key_rijndael256, bench_nonce_rijndael256,
                               pt_buf, msg_len, ct_buf);
    block_calls = probe_rijndael256_block_calls_decrypt_only(msg_len);
...
void bench_gcm_all(void)
{
    ...
    for (int s = 0; s < NUM_SIZES; ++s) {
        size_t msg_len = bench_sizes[s];

        printk("[AES-128-GCM]  msg=%u  ad=%d  iters=%d\n",
               (unsigned)msg_len, AD_LEN, BENCH_ITERS);
        printk("  %-16s  %10s    %12s\n", "operation", "cycles", "ns");
        printk("  [Our implementation]\n");
        bench_128_auth_only(msg_len);
        bench_128_verify_only(msg_len);
        bench_128_encrypt_only(msg_len);
        bench_128_decrypt_only(msg_len);
        printk("  [PSA Crypto]\n");
        bench_128_psa_encrypt(msg_len);
        bench_128_psa_decrypt(msg_len);
        printk("\n");

        printk("[AES-256-GCM]  msg=%u  ad=%d  iters=%d\n",
               (unsigned)msg_len, AD_LEN, BENCH_ITERS);
        printk("  %-16s  %10s    %12s\n", "operation", "cycles", "ns");
        printk("  [Our implementation]\n");
        bench_256_auth_only(msg_len);
        bench_256_verify_only(msg_len);
        bench_256_encrypt_only(msg_len);
        bench_256_decrypt_only(msg_len);
        printk("  [PSA Crypto]\n");
        bench_256_psa_encrypt(msg_len);
        bench_256_psa_decrypt(msg_len);
        printk("\n");

        printk("[Rijndael-256-GCM]  msg=%u  ad=%d  iters=%d\n",
               (unsigned)msg_len, AD_LEN, BENCH_ITERS);
        printk("  %-16s  %10s    %12s\n", "operation", "cycles", "ns");
        bench_rijndael256_auth_only(msg_len);
