/* Bench implementation extracted from main.c for Butterknife SAFE */
#include <string.h>
...
void verify_correctness(void)
{
    ...
    butterknife_safe_encrypt(bench_key, bench_nonce, plaintext, len, ct, tag);
    printk("PT:  %s\n", plaintext);
    print_hex("CT:  ", ct, len);
    print_hex("TAG: ", tag, 16);

    int rc = butterknife_safe_decrypt(bench_key, bench_nonce, ct, len, tag, dec);
    dec[len] = '\0';
    printk("DEC: %s\n", dec);
    printk("TAG verify: %s\n", rc == 0 ? "OK" : "FAIL");
    printk("--- OK ---\n\n");
...
void bench_block_encrypt(void)
{
    ...
    printk("[BUTTERKNIFE] TBC Encrypt (1 block, 1 branch):  %llu cycles  |  %llu ns\n",
...
{
    uint8_t tweak[16] = {0}, in[16] = {0}, out_b1[16], out_b2[16];
    timing_t start, end;
    uint64_t total_cycles = 0, total_ns = 0;

    for (int i = 0; i < WARMUP_ITERS; i++)
        butterknife_tbc_fork(bench_key, tweak, in, out_b1, out_b2);
    ...
    printk("[BUTTERKNIFE] TBC Fork  (1 block, 2 branches): %llu cycles  |  %llu ns\n",
           (unsigned long long)(total_cycles / BENCH_ITERS),
           (unsigned long long)(total_ns / BENCH_ITERS));
}
...
void bench_safe_encrypt(void)
{
    ...
    printk("[BUTTERKNIFE-SAFE] Encrypt  (%d iterations)\n", BENCH_ITERS);
    ...
    for (int s = 0; s < NUM_SIZES; s++) {
        ...
        for (int i = 0; i < WARMUP_ITERS; i++)
            butterknife_safe_encrypt(bench_key, bench_nonce,
...
void bench_safe_decrypt(void)
{
    ...
    printk("[BUTTERKNIFE-SAFE] Decrypt  (%d iterations)\n", BENCH_ITERS);
    printk("  %6s  %12s  %12s  %12s\n", "bytes", "cycles", "ns", "cycles/byte");
    ...
    for (int s = 0; s < NUM_SIZES; s++) {
        ...
        for (int i = 0; i < WARMUP_ITERS; i++)
            butterknife_safe_decrypt(bench_key, bench_nonce,
