 ...
 * Foundation, Inc., 51 Franklin Street, Fifth Floor, Boston, MAF
 * 02110-1301, USA.
 *
 */
...
/* Number of bits in the TWEAKEY state (192, 256 or 384) */
...
static void set_nonce_in_tweak_64(uint8_t *tweak, const uint8_t *nonce){
    tweak[0] = (tweak[0]&0xf0)     ^ (nonce[0] >> 4);
    tweak[1] = (nonce[0]&0xf) << 4 ^ (nonce[1] >> 4);
    tweak[2] = (nonce[1]&0xf) << 4 ^ (nonce[2] >> 4);
    tweak[3] = (nonce[2]&0xf) << 4 ^ (nonce[3] >> 4);
    tweak[4] = (nonce[3]&0xf) << 4 ^ (nonce[4] >> 4);
    tweak[5] = (nonce[4]&0xf) << 4;
}
...
static void set_block_number_in_tweak_64(uint8_t *tweak, const uint64_t block_no){
    tweak[5] = (tweak[5]&0xf0) ^ ((block_no >> 16ULL) & 0xf);
...
** In the case of Deoxys-BC-192, the tweakey word is composed of KEY || TWEAK.
...
// Because for some reason even this was hard coded
static void xor_values_64(uint8_t *v1, const uint8_t *v2) {
    int i;
...
void deoxys_I_aead_encrypt_64(const uint8_t *message, size_t m_len,
                              ...
                              uint8_t *ciphertext, size_t *c_len)
{
    ...
    skinny_64_192_init_keypart(&tks1, key, SKINNY_64_192_ROUNDS);

    /* Message */
    memset(tweak, 0, sizeof(tweak));
    set_nonce_in_tweak_64(tweak, nonce);
    
    memset(Checksum, 0, 8);
    set_stage_in_tweak(tweak, MSB_M);
    i=0;
    while (8*(i+1) <= m_len) {
        xor_values_64(Checksum, message+8*i );
        set_block_number_in_tweak_64(tweak, i );
        skinny_64_192_init_tweakpart(&tks2, tweak, SKINNY_64_192_ROUNDS);
        skinny_64_192_encrypt_with_tks(&tks1, &tks2, ciphertext + 8*i, message + 8*i);
        i++;
    }
    ...
    if (m_len > 8*i) {
        ...
        xor_values_64(Checksum, last_block);
        ...
        set_block_number_in_tweak_64(tweak, i);
        skinny_64_192_init_tweakpart(&tks2, tweak, SKINNY_64_192_ROUNDS);
        skinny_64_192_encrypt_with_tks(&tks1, &tks2, Pad, zero_block);
        ...
        set_block_number_in_tweak_64(tweak, i);
        skinny_64_192_init_tweakpart(&tks2, tweak, SKINNY_64_192_ROUNDS);
        skinny_64_192_encrypt_with_tks(&tks1, &tks2, Final, Checksum);
    }
    else{
        set_block_number_in_tweak_64(tweak, i);
                               ...
                               uint8_t *ciphertext, size_t *c_len)
{
    ...
    while (16*(i+1) <= m_len) {
        ...
        skinny_128_256_encrypt_with_tks(&tks1, &tks2, ciphertext + 16*i, message + 16*i);
        i++;
    }
    ...
    if (m_len > 16*i) {
        ...
        set_block_number_in_tweak(tweak, i);
        skinny_128_256_init_tk2(&tks2, tweak, SKINNY_128_256_ROUNDS);
        skinny_128_256_encrypt_with_tks(&tks1, &tks2, Pad, zero_block);
        ...
        set_block_number_in_tweak(tweak, i);
        skinny_128_256_init_tk2(&tks2, tweak, SKINNY_128_256_ROUNDS);
        skinny_128_256_encrypt_with_tks(&tks1, &tks2, Final, Checksum);
    }
    else{
        set_block_number_in_tweak(tweak, i);
        set_stage_in_tweak(tweak, MSB_CHKSUM_FULL);
                              ...
                              const uint8_t *ciphertext, size_t c_len)
{
    ...
    while(16*(i+1)<=c_len) {
        ...
        set_block_number_in_tweak(tweak, i);
        set_tweak_in_tweakey(tweakey, tweak);
        skinny_128_256_encrypt(tweakey, ciphertext + 16*i, message + 16*i);
        xor_values(Checksum, message+16*i);
        i++;
    }
    ...
    if (c_len == 16*i) {
        ...
        set_stage_in_tweak(tweak, MSB_CHKSUM_FULL);
        set_tweak_in_tweakey(tweakey, tweak);
        skinny_128_256_encrypt(tweakey, Final, Checksum);
        xor_values(Final, Auth);
        
        /* If the tags does not match, return error 0 */
        ...
        /* Encrypt */
        skinny_128_256_encrypt(tweakey, Pad, zero_block);
